...
public class ScreenShake : MonoBehaviour {
	...
	[Tooltip("do u want it to b always shaking?")]
	[SerializeField] private bool constantLoop;
	[Tooltip("constant shake intensity")]
	[SerializeField] public float constantIntensity;
	[Tooltip("should the constant shake happen in realtime?")]
	[SerializeField] private bool constantShakeRealTime;
	[Space]
	...
	private IEnumerator InfiniteShake() {
		if (!enable)
			yield break;
		while (true) {
			Vector2 dir = new Vector2(Random.Range(-1f, 1f), Random.Range(-1f, 1f)).normalized;
			Push(dir * constantIntensity);
			yield return StartCoroutine(PauseableWaitForSeconds(timeBetweenShakes, constantShakeRealTime));
		}
	}
